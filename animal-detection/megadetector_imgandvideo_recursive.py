...
classification_model = tf.keras.models.load_model('C:\yamaneko-kenkyu/animal-clasification/animal_classifier_binary_model.h5')
...
def process_detections(directory_path, results, all_images, every_n_frames):
    ...
    # Track highest confidence score and check if any detection exceeds the threshold
    max_confidence = 0
    best_frame = None
    ...
    # Save the confidence level of the best detection
    video_path = os.path.relpath(directory_path, output_base).replace('\\', '/')
    video_confidence_dict[video_path] = max_confidence

    # If no detection exceeds the threshold and this is a video, try re-running with more frames
    if not detection_above_threshold and every_n_frames > min_frames_interval:
        if any(file.endswith(('.mp4', '.avi', '.mov')) for file in os.listdir(directory_path)):  # Only run for videos
...
# Handle images: create folder, move image, run MegaDetector, classify
def process_images():
    ...
    for root, dir, files in os.walk(input_folder):
        for file in files:
            if file.lower().endswith(('.png', '.jpg', '.jpeg')):
                ...
                # Create a folder in the output directory named after the image (without extension)
                image_folder = os.path.join(image_path.replace('data','p_data'), os.path.splitext(file)[0])
                
                if not os.path.exists(image_folder):
                    os.makedirs(image_folder)
                
                # Move the image to this new folder
                new_image_path = os.path.join(image_folder, file)
                shutil.copy2(image_path, new_image_path)
                
                print(f"Copied image {file} to {new_image_path}")
